...
class RawDataLoader(SpanExtractionRawDataLoader):
    def __init__(self, data_path):
        ...
        self.train_file_name = ["HotpotQA.jsonl","NewsQA.jsonl", "SearchQA.jsonl","SQuAD.jsonl" ,"TriviaQA.jsonl"]
        self.test_file_name = ["HotpotQA_dev.jsonl","NewsQA_dev.jsonl","SearchQA_dev.jsonl","SQuAD_dev.jsonl","TriviaQA_dev.jsonl"]
    ...
    def load_data(self):
        if len(self.context_X) == 0 or len(self.question_X) == 0 or len(self.Y) == 0:
            ...
            for train_dataset in self.train_file_name:
                ...
                train_size += self.process_data_file(os.path.join(self.data_path, train_dataset),label,True)
            for test_dataset in self.test_file_name:
                label = test_dataset.split("-")[0]
                test_size += self.process_data_file(os.path.join(self.data_path, test_dataset),label,False)
            ...
            for line in f:
                ...
                for question in paragraph['qas']:
                    ...
                    for answer in question['detected_answers']: # same answer continue or not?
                        ...
                        # clean context data 
                        context = ''.join(filter(lambda x: x in printable, paragraph['context']))
