...
import data.raw_data_loader.NLPCC_2018.data_loader
import data.raw_data_loader.MRQA.data_loader
...
def test_raw_data_loader(args, dataset_name):
    ...
    elif dataset_name.startswith("MRQA"):
        data_loader = data.raw_data_loader.MRQA.data_loader.RawDataLoader(args.data_dir)
    elif dataset_name.startswith("nlpcc"):
        data_loader = data.raw_data_loader.NLPCC_2018.data_loader.RawDataLoader(args.data_dir)
    else:
        raise Exception("No such dataset!!")
    logger.info("Start loading data from files")
    # if dataset_name.startswith("nlpcc"):
    #     data_loader.load_data('result_insert.txt')
    #     data_loader.generate_h5_file(args.h5_file_path%('insert'))
    #
    #     data_loader.load_data('result_delete.txt')
    #     data_loader.generate_h5_file(args.h5_file_path%('delete'))
    #
    #     data_loader.load_data('result_substitution.txt')
    #     data_loader.generate_h5_file(args.h5_file_path%('substitution'))
    #
    #     data_loader.load_data('result_paraphrase.txt')
    #     data_loader.generate_h5_file(args.h5_file_path%('paraphrase'))
    #
    #     data_loader.load_data('test.txt')
    #     data_loader.generate_h5_file(args.h5_file_path % ('test'))
    #
    #     logger.info("Finish generating h5 data file")
    # else:
    start = time.time()
