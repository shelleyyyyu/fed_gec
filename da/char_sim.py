...
class CharFuncs(object):
    ...
    def comp_similarity(self, str1, str2, weights=(0.5, 0.5)):
        comp_similarity = 0
        if len(str1) == 0 or len(str2) == 0 or len(str1) != len(str2):
            return comp_similarity
        for i in range(0, len(str1)):
            char1 = str1[i]
            char2 = str2[i]
            shape_similarity = self.shape_similarity(char1, char2)
            pronunciation_similarity = self.pronunciation_similarity(char1, char2)
            comp_similarity += shape_similarity*weights[0] + pronunciation_similarity*weights[1]
        comp_similarity /= min(len(str1), len(str2))
        #print('comp_similarity str1=%s str2=%s comp_similarity=%s' % (str1, str2, comp_similarity))
        return comp_similarity
    ...
    def similarity(self, char1, char2, weights=(0.8, 0.2, 0.0), as_tree=False):
        ...
        if char1 in self.char_dict and char2 in self.char_dict:
            ...
            #print('char1=%s char2=%s shape_sim=%s sound_sim=%s freq_sim=%s' % (char1, char2, shape_sim, sound_sim, freq_sim))
