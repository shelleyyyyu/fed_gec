    ...
    def forward(
        ...
        **unused,
        ...
        if labels is not None:
            ...
            # loss_sep_fct = CrossEntropyLoss(reduction='none')
            # TODO(SS): do we need to ignore pad tokens in labels?
            masked_lm_loss = F.cross_entropy(lm_logits.view(-1, self.config.vocab_size), labels.view(-1))
            masked_lm_loss_list = F.cross_entropy(lm_logits.view(-1, self.config.vocab_size), labels.view(-1), reduction='none')
            masked_lm_loss_list = masked_lm_loss_list.view(outputs[0].size()[0], outputs[0].size()[1])
            masked_lm_loss_list = torch.mean(masked_lm_loss_list, dim=1)
            masked_lm_loss_list = masked_lm_loss_list.tolist()
            
        if not return_dict:
            output = (lm_logits,) + outputs[1:]
            return ((masked_lm_loss, masked_lm_loss_list) + output) if masked_lm_loss is not None else output
