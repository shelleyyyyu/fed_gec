...
class FedOptTrainer(object):
    ...
    def update_dataset(self, client_index, train_data_loss_list, train_data_list, if_augment='0', augment_percentage=0.1):
        ...
        logging.info('Before Augmentation: %s; len of examples: %d' %(str(client_index), len(train_data_loss_list)))
    ...
    def data_augmentation(self, train_local, client_index, train_data_loss_list, train_data_list, if_augment='0', augment_percentage=0.1):
        ...
        if if_augment == '1' and len(train_data_loss_list) > 1 and len(train_data_list) > 1:
            ...
            # <data_preprocessing.base.base_data_loader.BaseDataLoader object at 0x7fbfe49f9a10>
            aug_train_data = {}
            aug_train_data['X'] = []
            aug_train_data['y'] = []
            ...
            logging.info('sort_train_indicessort_train_indicessort_train_indicessort_train_indices')
            logging.info(sort_train_indices)
            #[16  3  4  8 14  7 17  5  9  1 15 12 13  0  2  6 11 19 18 10]
            #for i in range(len(train_local.examples)):
            to_augment_threadshold = int(len(sort_train_indices) * augment_percentage)
            logging.info('to_augment_threadsholdto_augment_threadsholdto_augment_threadshold')
            logging.info(to_augment_threadshold)
            for idx, i in enumerate(sort_train_indices):
                ...
                # auto_augment_input_data = train_data_list[i]
                # aug_train_data['X'].append(auto_augment_input_data)
                # aug_train_data['y'].append(output_data)
    ...
    def train(self, round_idx = None):
        ...
        # logging.info(self.trainer) # training.fed_trainer_transformer.FedTransformerTrainer
        train_data_loss_list, train_data_list = self.trainer.train(self.train_local, self.device, self.args)
        weights = self.trainer.get_model_params()

        # transform Tensor to list
        if self.args.is_mobile == 1:
            weights = transform_tensor_to_list(weights)
        return weights, self.local_sample_number, train_data_loss_list, train_data_list
