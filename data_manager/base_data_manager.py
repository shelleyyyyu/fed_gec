...
class BaseDataManager(ABC):
    ...
    def __init__(self, args, model_args, process_id, num_workers):
        ...
        self.num_clients = 4 #self.load_num_clients(self.args.partition_file_path, self.args.partition_method)
    ...
    def load_num_clients(partition_file_path, partition_name):
        ...
        # print(list(data_file[partition_name].keys()))
        # print(data_file[partition_name]['n_clients'][()])
        # print(data_file[partition_name]['alpha'][()])
        # print(list(data_file[partition_name]['partition_data'].get('/niid_cluster_clients=100_alpha=0.1/partition_data').items())[0][0])
        # print(list(data_file[partition_name]['partition_data'].get('/niid_cluster_clients=100_alpha=0.1/partition_data/0').items()))
        # print(list(list(data_file[partition_name]['partition_data'].get('/niid_cluster_clients=100_alpha=0.1/partition_data/0').items())[0])[0])
        # print(list(list(list(data_file[partition_name]['partition_data'].get('/niid_cluster_clients=100_alpha=0.1/partition_data/0').items())[0])[1]))
        # print(list(list(data_file[partition_name]['partition_data'].get('/niid_cluster_clients=100_alpha=0.1/partition_data/0').items())[1]))
        # print(list(data_file[partition_name]['partition_data'].get('/niid_cluster_clients=100_alpha=0.1/partition_data').items())[1])
        # print(list(data_file[partition_name]['partition_data'].get('/niid_cluster_clients=100_alpha=0.1/partition_data').items())[2])
        # exit()
    ...
    def _simulated_sampling(self, process_id):
        ...
        # print('self.args.comm_round', self.args.comm_round)
        ...
        for round_idx in range(self.args.comm_round):
            if self.num_clients == self.num_workers:
                ...
                client_indexes = np.random.choice(
                    ...
                    nc, replace=False)
                #logging.info("process_id = %s; client_indexes = %s" %(str(process_id), str(client_indexes)))
    ...
    def load_centralized_data(self, cut_off=None):
        ...
        if state:
            ...
            train_index_list = []
            test_index_list = []
            for client_idx in tqdm(partition_file[partition_method]["partition_data"].keys(), desc="Loading index from h5 file."):
            ...
            # print((train_index_list[1]))
            # print((train_index_list[3]))
    ...
    def _load_federated_data_server(self, test_only=True, test_cut_off=None):
        ...
        print('----- loading global federated data -----')
        if state:
            train_examples, train_features, train_dataset, test_examples, test_features, test_dataset = res
    ...
    def _load_federated_data_local(self):
        ...
        for client_idx in self.client_index_list:
            print('----- loading local federated data - client_idx', client_idx, '-----')
            ...
            if state:
                ...
                logging.info('-'*5+'Train Example'+'-'*5)
                logging.info('X', train_data['X'][0])
                logging.info('Y', train_data['y'][0])
                logging.info(len(train_data['X']))
                logging.info('-'*5+'Text Example'+'-'*5)
                logging.info('X', test_data['X'][0])
                logging.info('Y', test_data['y'][0])
